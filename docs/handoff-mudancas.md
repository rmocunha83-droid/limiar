...
- **QA:** `-LimiarForceSubscriptionGate -LimiarGateTrialEligible` abre o portão; `-LimiarForceGateRecovery` abre a recuperação (DEBUG). O scheme Debug agora usa `Limiar.storekit` para validar localmente os três produtos; sandbox/TestFlight continuam sendo a confirmação dos dados reais da App Store.
...
## 0c. Oferta de boas-vindas após cancelamento — 23/08/2026 (branch `codex/oferta-boas-vindas`)

- **Fluxo:** a primeira recuperação após a pessoa fechar a folha da Apple pode mostrar “Fique pelo preço do anual, sem o compromisso do anual”. O produto separado `limiar_premium_monthly_welcome` mantém período mensal e teste grátis de 7 dias; nunca aparece no portão, paywall legado, Configurações ou winback.
- **Layout da oferta:** a tela inteira rola como o portão principal. O CTA, “Já sou assinante”, texto legal e links ficam logo após “Esta oferta aparece uma vez…”, sem barra inferior fixa, fundo opaco ou divisor. Os atalhos “Prefiro o anual…” e “Ver todos os planos” foram retirados dessa variante.
- **Escassez verdadeira:** `limiar.offer.welcome.consumed` fica no `UserDefaults` do app group e é espelhado no Keychain para sobreviver à reinstalação. A oferta só fica disponível com recuperação visível, os três produtos carregados, relação de preços coerente com anual ÷ 12, teste introdutório elegível, nenhum histórico de assinatura e `WelcomeOfferPolicy.isEnabled == true`. Aceitar ou mandar o app ao background com a oferta visível consome a oportunidade definitivamente no aparelho.
- **Fallback seguro:** produto ausente, preço incoerente ou inelegibilidade mostra a recuperação comum, sem preço fictício nem “Carregando oferta”. Se a pessoa cancelar também a compra de boas-vindas, a nova recuperação é comum e o plano selecionado volta ao mensal normal. Todo desfecho da tentativa promocional também sanitiza a seleção para impedir que CTAs públicos recompren o SKU oculto.
- **Medição:** Firebase recebe `gate_offer_viewed`, `gate_offer_accepted` e, ao sair para o background, `gate_offer_declined{reason: background}`. O ciclo de compra existente registra `plan=monthly_welcome`; a Meta não recebe preço nem plano novo.
- **Avaliação:** comparar `trials recuperados pela oferta × desconto mensal` com o custo por trial do tráfego pago e acompanhar `gate_offer_accepted / gate_offer_viewed`. Se a oferta recuperar menos de 5% de quem a vê, desligar mudando `WelcomeOfferPolicy.isEnabled` para `false` e publicar nova build.
- **Preço e território:** Romeu confirmou em 23/08/2026 o price point de **R$ 7,50/mês**, exclusivamente para o Brasil.
- **App Store Connect:** produto `limiar_premium_monthly_welcome` criado no grupo `Limiar Premium` (Apple ID `6804473230`), no mesmo nível do mensal. Disponibilidade: Brasil (1/175); preço: R$ 7,50; teste grátis: primeira semana desde 23/08/2026, sem data final. Localização pt-BR salva como `Limiar Premium Mensal` / `Acesso completo ao Limiar. Mensal pelo preço do anual.` (o texto proposto maior não cabia no limite de 55 caracteres). Status `Preparar para envio`, sem adição para revisão.
- **QA local:** `Limiar.storekit` contém os três produtos com teste grátis de 7 dias e preço local de boas-vindas em R$ 7,50, o price point oficial mais próximo encontrado no ASC; está associado somente ao `LaunchAction` Debug do scheme. Usar `-LimiarForceSubscriptionGate -LimiarGateTrialEligible -LimiarForceGateRecovery -LimiarForceWelcomeOffer`; a flag de oferta respeita a ausência do produto e não ressuscita uma oferta já consumida.
- **QA automatizado:** a suíte completa no iPhone SE terminou com `TEST SUCCEEDED`, incluindo telemetria terminal exclusiva e políticas da oferta. A captura visual ficou pendente porque o runtime iOS 26.5 travou no `00LaunchServicesMigrator` após o reset do simulador dedicado; nenhum dado sintético foi injetado para mascarar a indisponibilidade.
